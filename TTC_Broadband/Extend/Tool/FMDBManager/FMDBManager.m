...
-(void)insertAndUpdateModelToDatabase:(id)model withCheckNum:(int)checkNum{
    [_dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        ...
        //        }
        //拼接查询语句
        NSString *selectStr = [NSMutableString stringWithFormat:@"select * from %@ where",KCLASS_NAME(model)];
        NSString *selectStrTail;
        if (KMODEL_PROPERTYS_COUNT > checkNum) {
        ...
        if([resultSet next])
        {
            ...
            NSString *updateStrTail = [NSString stringWithFormat:@" where "];
//            NSString *updateStrTailContent = [NSString string];
            for (int i = 0; i< checkNum; i++) {
                updateStrTail = [updateStrTail stringByAppendingFormat:@"%@ = '%@'",[KMODEL_PROPERTYS objectAtIndex:i],[model valueForKey:[KMODEL_PROPERTYS objectAtIndex:i]]];
                if (i != (checkNum-1)) {
                    updateStrTail = [updateStrTail stringByAppendingFormat:@" and "];
                }
            }
            // 整句拼接更新语句
