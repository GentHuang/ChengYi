...
- (void)updateLabelForLayer:(SliceLayer *)pieLayer value:(CGFloat)value titleName:(NSString*)title;
...
- (void)reloadData
{
    if (_dataSource)
    {
        ...
        //动画的开始
        [CATransaction begin];
        [CATransaction setAnimationDuration:_animationSpeed];//_animationSpeed
        ...
        {
            for(SliceLayer *layer in _pieView.layer.sublayers){
                [self updateLabelForLayer:layer value:0 titleName:@""];
        ...
        for(int index = 0; index < sliceCount; index ++)
        {
            ...
            //添加名字
            NSLog(@"==== %d",index);
            [self updateLabelForLayer:layer value:values[index] titleName:_titleArray[index]];
            [layer createArcAnimationForKey:@"startAngle"
...
- (void)updateLabelForLayer:(SliceLayer *)pieLayer value:(CGFloat)value titleName:(NSString *)title
{
    ...
    NSString *label;
    if(_showPercentage)
        label = [NSString stringWithFormat:@"%@\n%0.01f%%",title,pieLayer.percentage*100];
    else
        label = (pieLayer.text)?pieLayer.text:[NSString stringWithFormat:@"%@\n%0.01f%%",title,value];
    ...
    {
        [textLayer setString:label];
        [textLayer setBounds:CGRectMake(0, 0, size.width, 40)];
    }
    [CATransaction setDisableActions:NO];
}
